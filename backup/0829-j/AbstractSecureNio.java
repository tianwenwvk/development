...
public abstract class AbstractSecureNio extends AbstractExecutionThreadService {
	...
	protected static boolean DEFAULT_USING_SSL = false;
	protected static String[] DEFAULT_PROTOCOLS = {"TLSv1.2"};
	protected static String[] DEFAULT_CIPHER_SUITS = {"TLS_RSA_WITH_AES_256_GCM_SHA384"};
	
	public static boolean usingSSL = PropertiesReader.getUsingSSL(DEFAULT_USING_SSL);
	protected static String[] protocols = PropertiesReader.getProtocols(DEFAULT_PROTOCOLS);
	protected static String[] cipherSuits = PropertiesReader.getCipherSuites(DEFAULT_CIPHER_SUITS);
	...
	private SSLContext setupSSLContext() {
		...
		try {
			String trustStoreLoc = PropertiesReader.getTrustStoreLoc();
			char[] tsPassPhrase = PropertiesReader.getTrustStorePassPhrase().toCharArray();
		...
		try {
			String keyStoreLoc = PropertiesReader.getKeyStoreLoc();
			char[] ksPassPhrase = PropertiesReader.getKeyStorePassPhrase().toCharArray();
   ...
   protected SSLEngine setupEngine(String peerHost, int peerPort) {
       ...
       //log.info("-----------------getSupportedCipherSuites: "+ Arrays.toString( engine.getSupportedCipherSuites() ));
