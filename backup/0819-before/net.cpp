...
bool GetPeerSSL()
{
	return GetBoolArg("-peerssl", false);
}
...
static bool sslctx_s_init()
{
	...
	g_s_sslctx = SSL_CTX_new(TLSv1_2_server_method());//(SSLv23_server_method());//(TLSv1_server_method());
	if (!g_s_sslctx) {
	...
	SSL_CTX_set_session_id_context(g_s_sslctx,(const unsigned char *)ssl_session_ctx_id.c_str(), sizeof(ssl_session_ctx_id));
	...
	SSL_CTX_set_verify_depth(g_s_sslctx, 4);
	SSL_CTX_load_verify_locations(g_s_sslctx, peer_ca_file.c_str(), NULL);
	/* Set the key and cert */
//	std::string key_passwd = "bitcoin";
//	SSL_CTX_set_default_passwd_cb(g_s_sslctx, passwd_func);
//	SSL_CTX_set_default_passwd_cb_userdata(g_s_sslctx, (void *)password);
...
static bool sslctx_c_init()
{
    ...
    g_c_sslctx = SSL_CTX_new(SSLv23_client_method());//(TLSv1_client_method());
    if(g_c_sslctx == NULL){
    ...
    //SSL_CTX_set_default_passwd_cb(g_c_sslctx,KEY_PASSWD);
...
static void AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if (r <= 0) {
    	...
    	std::cout << "[DEBUG]     Error string : "
    	                  << ERR_error_string( err_SSL_get_error, NULL ) << std::endl;
